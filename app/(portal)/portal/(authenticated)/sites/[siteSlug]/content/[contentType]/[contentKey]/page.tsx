...
export default async function ContentEntryPage({ params }: { params: Promise<{ siteSlug: string; contentType: string; contentKey: string }> }) {
  ...
  if (siteSlug === "alford-custom-homes" && contentType === "page_section" && contentKey === "homepage-hero") {
    ...
    return <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#9a5f34]">Homepage</p><h1 className="mt-2 mb-8 font-display text-4xl font-semibold">Homepage hero</h1><HomepageHeroEditor entry={{ ...entry, draft_data: validated.data }} siteSlug={siteSlug} siteName={site.name} canPublish={canPublish(access)} /></div>;
  }
  const definition = getSiteDefinition(siteSlug);
  const modelKey = definition?.modelKeyForEntry(contentType, contentKey) ?? null;
  const model = modelKey ? definition?.models[modelKey] : null;
  const groups = modelKey ? definition?.editorGroups[modelKey] : null;
  if (modelKey && model && groups) {
    ...
    return <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#9a5f34]">{model.label}</p><h1 className="mt-2 mb-8 font-display text-4xl font-semibold">{entry.title}</h1><StructuredEntryEditor entry={{ ...entry, draft_data: validated.data as Record<string, unknown> }} siteSlug={siteSlug} siteName={site.name} modelKey={modelKey} groups={groups} previewPath={model.previewPath(validated.data)} canPublish={canPublish(access)} /></div>;
  }

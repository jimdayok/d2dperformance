...
import { generateWebsiteFeedbackPdf } from "@/lib/website-feedback-report";
import { normalizeReviewUrl, overallQuestions, pageQuestions, type MarkupNote, type PageFeedbackInput, type SubmitFeedbackInput } from "@/lib/website-feedback-schema";
...
export async function getFeedbackSession(sessionId: string, accessToken: string) {
  ...
  const supabase = createSupabaseAdminClient();
  const { data: pages, error } = await supabase.from(PAGE_TABLE).select("id,url,page_title,answers,ratings,annotations,page_score,commentary,created_at,updated_at").eq("session_id", sessionId).order("updated_at", { ascending: true });
  if (error) throw error;
...
export async function saveFeedbackPage(sessionId: string, accessToken: string, page: PageFeedbackInput) {
  ...
  }, { onConflict: "session_id,url" }).select("id,url,page_title,answers,ratings,annotations,page_score,commentary,created_at,updated_at").single();
  if (error) throw error;
...
function pageAnnotations(page: Record<string, unknown>) {
  return Array.isArray(page.annotations) ? page.annotations as MarkupNote[] : [];
}

function annotationsToText(annotations: MarkupNote[]) {
  if (!annotations.length) return "No visual markup added.";
  return annotations.map((annotation, index) => `${index + 1}. ${annotationLabels[annotation.category]}: ${annotation.text || "Area marked without an additional note."}`).join("\n");
}

function annotationsToHtml(annotations: MarkupNote[]) {
  if (!annotations.length) return "";
  const items = annotations.map((annotation, index) => `<li style="margin:0 0 12px"><strong style="color:#0c7379">${index + 1}. ${annotationLabels[annotation.category]}</strong><br /><span style="color:#35423f">${escapeHtml(annotation.text || "Area marked without an additional note.").replaceAll("\n", "<br />")}</span></li>`).join("");
  return `<div style="margin:22px 0;padding:18px;background:#edf4f1;border-left:4px solid #0c7379"><p style="margin:0 0 10px;color:#18201d;font-size:12px;font-weight:700;text-transform:uppercase">Website markup · ${annotations.length} ${annotations.length === 1 ? "mark" : "marks"}</p><ol style="margin:0;padding-left:20px;font-size:15px;line-height:1.55">${items}</ol></div>`;
}
...
async function sendFeedbackEmail(session: Record<string, unknown>, pages: Array<Record<string, unknown>>, overall: SubmitFeedbackInput) {
  ...
  const pdf = await generateWebsiteFeedbackPdf(session, pages, overall);
  const reportFilename = `${String(session.company || "website").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60) || "website"}-feedback.pdf`;
  const approvalLabel = overall.approvalStatus.replaceAll("_", " ");
  const pageText = pages.map((page, index) => `PAGE ${index + 1}: ${page.page_title || page.url}\nURL: ${page.url}\nPage score: ${page.page_score}/5\n\nWEBSITE MARKUP\n${annotationsToText(pageAnnotations(page))}\n\nOPTIONAL PAGE SURVEY\n${answersToText((page.answers ?? {}) as Record<string, string>, pageQuestionLabels)}\nGeneral page notes: ${page.commentary || "None"}`).join("\n\n---\n\n");
  ...
  const deliveries = await Promise.all(recipients.map((recipient) => resend.emails.send({
    from,
    to: recipient.email,
    ...
    text: `${recipient.role === "client" ? "Thank you for reviewing the site. This is your copy of the feedback sent to the D2D Digital and Marketing teams.\n\n" : "A client website review is ready.\n\n"}The marked-up website PDF is attached.\n\n${baseText}`,
    html: buildHtml(recipient.role),
    attachments: [{ filename: reportFilename, content: pdf.toString("base64"), contentType: "application/pdf" }],
  }, { idempotencyKey: `website-feedback-${String(session.id)}-${recipient.role}` })));
  const rejected = deliveries.find((delivery) => delivery.error)?.error;
  if (rejected) throw new Error(`Feedback email delivery was rejected: ${rejected.message}`);
}

...
export function HomepageHeroEditor({ entry, siteSlug, siteName, canPublish }: { entry: Entry; siteSlug: string; siteName: string; canPublish: boolean }) {
  ...
  async function save(values: HomepageHero) {
    setPending(true); setErrorMessage(""); setMessage("Saving draft…");
    ...
    if (!response.ok) { setErrorMessage(response.status === 409 ? "Someone else saved a newer draft. Refresh before continuing." : result.error ?? "Draft save failed."); return; }
  ...
  async function workflow(action: "submit" | "publish") {
    setPending(true); setErrorMessage(""); setMessage(action === "submit" ? "Submitting for review…" : "Publishing…");
    ...
    const result = await response.json(); setPending(false);
    if (!response.ok) { setErrorMessage(result.error ?? (action === "submit" ? "Submission failed." : "Publishing failed.")); return; }
    setMessage(result.message ?? (action === "submit" ? "Submitted for review." : "Published.")); setStatus(action === "submit" ? "in_review" : "published");
  }
  ...
  async function openFeedback() {
    ...
    if (response.ok && result.url) window.open(`${feedbackToolOrigin}/digital/website-feedback?site=${encodeURIComponent(siteSlug)}&url=${encodeURIComponent(result.url)}`, "_blank", "noopener,noreferrer"); else setErrorMessage(result.error ?? "The feedback workspace could not be opened.");
  }

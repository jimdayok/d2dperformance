      ...
      <div className="feedback-layout">
        <aside className="feedback-panel">
          ...
          <div className="feedback-questions">
            ...
            <div className="markup-count"><strong>{annotations.length}</strong><span>{annotations.length === 1 ? "mark on this page" : "marks on this page"}</span><small>Use the tools above the website to circle an area or attach a note.</small></div>
        ...
        <section className="feedback-preview" aria-label="Desktop website preview">
          ...
          <div className="preview-notice"><span>Browse to any page, then choose Circle an area or Place a note. Every page keeps its own markup and general notes.</span></div>
...
function StartReviewForm({ initialUrl, sourceSiteSlug, onSubmit, working, error }: { initialUrl: string; sourceSiteSlug: string; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void; working: boolean; error: string }) {
  return <main className="feedback-start"><div className="feedback-start-grid"><section><a href="/digital" className="feedback-brand"><span>D2D</span><strong>DIGITAL_</strong></a><p className="feedback-eyebrow">Client collaboration / Website iteration</p><h1>See the site.<br/><em>Shape what comes next.</em></h1><p className="feedback-start-lede">Circle the exact parts of each webpage you want changed, attach replacement copy or picture notes, and share one organized review with the D2D Digital and Marketing teams.</p><ol><li><span>01</span>Browse every page you want to review.</li><li><span>02</span>Circle areas and place notes directly on the site.</li><li><span>03</span>Save each page and share the complete review with the teams.</li></ol></section><form onSubmit={onSubmit}><div className="feedback-form-heading"><p>Start a private review</p><span>{sourceSiteSlug ? "Opened from Site Manager" : "D2D Digital feedback tool"}</span></div><label>Your name<input name="clientName" required minLength={2} autoComplete="name" /></label><label>Company<input name="company" required minLength={2} autoComplete="organization" /></label><label>Email<input name="clientEmail" type="email" required autoComplete="email" /></label><label>Website or preview URL<input name="initialUrl" type="url" required defaultValue={initialUrl} placeholder="https://preview.example.com" /></label><div className="altcha-wrap"><AltchaVerification /></div>{error ? <p className="feedback-error" role="alert">{error}</p> : null}<button type="submit" disabled={working}>{working ? <LoaderCircle className="animate-spin" /> : <ArrowUpRight />}{working ? "Creating review…" : "Open feedback workspace"}</button><small>Your review is private and protected by ALTCHA proof-of-work verification. No ad tracking or image puzzles.</small></form></div></main>;
}
